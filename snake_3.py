...
class GameObject:
    ...
    def __init__(self) -> None:
        ...
        # store position as a list of tuples so subclasses that expect a list (like Snake) match the type
...
class Snake(GameObject):
    ...
    def move(self) -> None:
        ...
        self.position.insert(0, ((x + self.direction[0] * GRID_SIZE) %
                                 SCREEN_WIDTH,
    ...
    def reset(self) -> None:
        ...
        self.next_direction = choice(ramdon_direction)
...
class Apple(GameObject):
    ...
    def __init__(self, forbidden_position) -> None:  # ВЫДАЕТ ОШИБКУ В ТЕСТЕ ПРИ ДОБАВ ПАРАМЕТРА
    ...
    def randomize_position(self, for_posit) -> tuple[int, int]:
        """Случайное расположение объекта яблоко"""
        while True:
            self.position = ((randint(0, GRID_WIDTH - 1) * GRID_SIZE),
                             (randint(0, GRID_HEIGHT - 1) * GRID_SIZE))
            if self.position not in for_posit:
                break
        return self.position
